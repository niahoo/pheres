...
class ChannelController extends Controller
{
    ...
    {
        $user = \Auth::user();
        $clients = $this->channelClients($channel, $user);
    ...
    private function channelClients(FeedChannel $channel, User $user)
    {
        $push = $user->apiClients->filter(function($client) use ($channel) {
            return $channel->allowsClientToPush($client);
        });
        $read = $user->apiClients->filter(function($client) use ($channel) {
            return $channel->allowsClientToRead($client);
        });
        return compact('read', 'push');
    }
